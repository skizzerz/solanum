...
static void standard_init(void)
{
	...
	/* keep UIDs alphabetical with nicknames for clearer sorting order in tests
	 * (and to ensure join order and sort order are slightly different to verify sorting works as expected) */
	...
	local_chan_v = make_local_person_id("LChanVoice", TEST_ME_ID "90004");
	local_chan_p = make_local_person_id("LChanPeon", TEST_ME_ID "90003");
	...
	remote_chan_v = make_remote_person_id(server, "RChanVoice", TEST_SERVER_ID "90104");
	remote_chan_p = make_remote_person_id(server, "RChanPeon", TEST_SERVER_ID "90103");
...
static void metadata_list__large_channel_destroy(void)
{
	...
	RB_RADIXTREE_FOREACH(msptr, &state, channel->members)
	{
		remove_user_from_channel(msptr);
	}
...
static void metadata_sync__channel(void)
{
	...
	/* for channel + member metadata, the channel metadata is listed first, then member in alphabetical order by UID */
