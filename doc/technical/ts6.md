...
- Legacy: exists for backwards compatibility with old IRCds; implementing it
  is not recommended
- Draft: functionality does not exist in mainline solanum, but describes how
  the implementation will look once it does. It may exist in a module in a
  different repository (e.g. Libera's solanum-extras repo). Subject to change
  in incompatible ways between now and if/when it becomes part of solanum.
...
tag. Unrecognized tags are stripped and not propagated further. Tags in
draft status are supported via extension modules found in the Libera
solanum-extras repository rather than being directly in upstream solanum.
