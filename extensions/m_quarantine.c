...
static void quarantine_priv_change(void *);
...
mapi_hfn_list_av1 quarantine_hfn_list[] = {
	...
	{ "priv_change", quarantine_priv_change },
...
quarantine_change_umode(void *data_)
{
	...
	{
		if (data->client == allowed_umode_change)
		{
			/* another hook or message handler flagged this mode change as allowed */
			if (IsQuarantined(data->client))
				sendto_one_notice(data->client, ":*** %s", EmptyString(apply_msg) ? DEFAULT_APPLY_MSG : apply_msg);
			else
				sendto_one_notice(data->client, ":*** %s", EmptyString(remove_msg) ? DEFAULT_APPLY_MSG : remove_msg);
		}
		else if (data->oldumodes & user_modes['q'])
 ...
 */
static void
quarantine_priv_change(void *data_)
{
...
quarantine_privmsg_user(void *data_)
{
	...
	{
		if (IsQuarantined(data->target_p))
		{
